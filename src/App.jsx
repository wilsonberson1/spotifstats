...
export default function App() {
  ...
  return (
    <>
      <header>
        <h1>Spofstats — Ano Corrente <small className="badge">aprox.</small></h1>
        <nav>
      ...
      <main>
        {!token && (
          <div className="card" style={{ marginBottom: 16 }}>
            <p>
              Entre com sua conta do Spotify para ver seus artistas e músicas mais ouvidas neste ano (aprox.)
              usando o <strong>Spofstats</strong>. Nenhum dado é armazenado.
            </p>
          </div>
        )}
            ...
            <div className="grid" style={{ marginBottom: 16 }}>
              ...
              <div className="card">
                ...
                <ul>
                  {tracks.map((t, i) => (
                    <li key={t.id}>
                      {i + 1}. {t.name} — <span style={{ color: '#64748b' }}>{(t.artists||[]).map(a => a.name).join(', ')}</span>
                    </li>
                  ))}
                </ul>
              </div>
      ...
      <footer>
        <p>
          © {new Date().getFullYear()} Spofstats — nenhum dado é armazenado. Ano corrente (aprox.) baseado na API do Spotify.
        </p>
      </footer>
    </>
  )
